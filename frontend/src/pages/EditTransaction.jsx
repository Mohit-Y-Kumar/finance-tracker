  ...
  const { token } = useContext(AuthContext); 
    ...
    return (
     ...
     relative overflow-hidden flex flex-col items-center justify-start">
       ...
       bg-purple-200 rounded-full opacity-30 animate-pulse"></div>
        <div className="absolute bottom-[-100px] right-[-50px]
       w-96 h-96 bg-pink-200 rounded-full opacity-20 animate-pulse"></div>

        {/* Header */}
        <h2 className="text-2xl sm:text-3xl font-extrabold mb-6
       flex items-center gap-2 text-gray-600 relative z-10">
       ...
       shadow-lg p-6 sm:p-10 relative z-10">
          <TransactionForm transactionId={id} token={token} />
        </div>
      </div>
    );
  };
